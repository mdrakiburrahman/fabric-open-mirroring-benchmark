...
def main():
    ...
    metric_client = MetricOperationsClient(mirroring_client=mirroring_client, host=args.host_root_fqdn, logger=logger)
    all_metrics = metric_client.get_all_metrics(schema_name=args.schema_name, table_name=args.table_name)
    metrics_data = {
        "metric_key": list(all_metrics.keys()),
        "metric_value": list(all_metrics.values())
    }
